...
// Delete the current text (manual delete)
document.getElementById("delete-btn").addEventListener("click", async () => {
    await deleteCurrentText();
});

// Function to delete current text from the server and update local array
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (currentIndex >= textData.length) {
                currentIndex = Math.max(0, textData.length - 1);
            }
            updateText();
        } else {
...
// Save recorded audio and then delete the current text
document.getElementById("save-btn").addEventListener("click", async () => {
    ...
    try {
        ...
        if (response.ok) {
            // After uploading, delete the current text
            await deleteCurrentText();
